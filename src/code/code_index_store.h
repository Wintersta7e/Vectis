...
[[nodiscard]] vectis::core::Result<void> save_index(vectis::services::StorageEngine& storage,
...
load_index(vectis::services::StorageEngine& storage, CodeIndex& index);

/// Check whether a cache exists for the given project root.
[[nodiscard]] bool has_cache_for(vectis::services::StorageEngine& storage,
                                 const std::filesystem::path& project_root);

/// Delete all index data from the database (files, symbols, deps,
/// fts_content, and cache.* kv_store entries).
[[nodiscard]] vectis::core::Result<void> clear_cache(vectis::services::StorageEngine& storage);
