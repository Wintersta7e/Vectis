...
void IndexEngine::index_file(std::int64_t file_id, std::string_view path, std::string_view content)
{
    if (m_impl->storage == nullptr) {
        return;
    }

    auto stmt = m_impl->storage->prepare("INSERT INTO fts_content (source, source_id, title, body) "
                                         "VALUES ('file', ?, ?, ?)");
    if (!stmt) {
...
void IndexEngine::remove_file(std::int64_t file_id)
{
    ...
    auto stmt =
        m_impl->storage->prepare("DELETE FROM fts_content WHERE source = 'file' AND source_id = ?");
    if (!stmt) {
        return;
    }
    stmt->bind(1, file_id);
    ...
    auto stmt2 =
        m_impl->storage->prepare("DELETE FROM fts_content WHERE source = 'symbol' AND source_id IN "
...
void IndexEngine::index_symbols(std::int64_t file_id,
                                const std::vector<vectis::code::Symbol>& symbols)
{
    ...
    for (const auto& sym : symbols) {
        ...
        const std::string_view body =
            sym.signature.empty() ? std::string_view{sym.name} : std::string_view{sym.signature};
        stmt->bind(3, body);
        if (auto r = stmt->execute(); !r) {
...
std::vector<SearchResult> run_search(StorageEngine* storage, std::string_view query,
                                     const char* source_filter, int max_results)
{
    std::vector<SearchResult> results;
    if (storage == nullptr || query.empty()) {
        return results;
    }

    const auto escaped = escape_fts_query(query);

    std::string sql = "SELECT source, source_id, title, "
...
int IndexEngine::indexed_file_count() const
{
    ...
    auto stmt = m_impl->storage->prepare("SELECT COUNT(*) FROM fts_content WHERE source = 'file'");
    if (!stmt) {
        return 0;
    }
    auto rows = stmt->query();
