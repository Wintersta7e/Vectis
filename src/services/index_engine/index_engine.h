...
public:
    ...
    void index_symbols(std::int64_t file_id, const std::vector<vectis::code::Symbol>& symbols);

    // ----- Search ------------------------------------------------------------

    /// Search across all sources (files + symbols).
    [[nodiscard]] std::vector<SearchResult> search(std::string_view query, int max_results = 20);

    /// Search file content only.
    [[nodiscard]] std::vector<SearchResult> search_files(std::string_view query,
                                                         int max_results = 20);
